...
**Note** Use `gemini-2.5-flash` or `gemini-2.5-pro` models. Gemini 1.5 models are now deprecated.
...
*   **Separate User Interaction from Sequential Workflows**: The root agent should not be a sequential agent if it needs to interact with the user first. Instead, the initial user interaction should be handled by a separate agent, which then hands off to a sequential agent for workflow execution.
